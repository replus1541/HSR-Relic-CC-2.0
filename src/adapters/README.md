...
Phase 5-A에서는 adapter 구현 전 contract를 문서로 고정합니다.
...
- Adapter는 `adapterId`, `sourceKind`, `load`, `normalize`, `report` 흐름을 따라야 합니다.
- Adapter report는 success/blocked/skipped/error count를 남겨야 합니다.
...
- 문서: `reports/adapter/adapter-contract.md`
- 다음 구현 단위: adapter registry skeleton

현재 이 폴더에는 실제 source parsing, normalization, calculation 로직이 없습니다.
